...
// TestServeRawOverTCPForwardsBothDirections checks the byte relay end to end
// through the exported API: bytes written to the local port arrive on the
// WebSocket as a binary frame, and frames sent back arrive on the local port.
func TestServeRawOverTCPForwardsBothDirections(t *testing.T) {
	...
	serveErr := make(chan error, 1)
	go func() { serveErr <- ServeRaw(ctx, listener, wsURL, "session-1", "dXNlcjprZXk=") }()

	conn, err := net.Dial("tcp", listener.Addr().String())
	if err != nil {
		t.Fatalf("dial the tcp socket: %v", err)
	}
	...
	// closure, and ServeRaw waits for its connection goroutines on the way out.
	_ = conn.Close()
	cancel()
	select {
	case err := <-serveErr:
		if err != nil {
			t.Errorf("ServeRaw returned %v, want nil after cancel", err)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("ServeRaw did not return after cancel")
	}
}

// TestServeRawReturnsNilOnContextCancel pins the shutdown contract an
...
func TestServeRawReturnsNilOnContextCancel(t *testing.T) {
	listener := listenLocal(t)

	ctx, cancel := context.WithCancel(context.Background())
	serveErr := make(chan error, 1)
	go func() { serveErr <- ServeRaw(ctx, listener, "ws://unused.invalid", "session-1", "dXNlcjprZXk=") }()
