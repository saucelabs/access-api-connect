...
// ServeRaw accepts connections on l and gives each one its own WebSocket,
// copying bytes both ways without interpreting them. That suits any endpoint
// carrying a protocol unwrapped: adbUrl on Android, where `adb connect <addr>`
// attaches, and usbmuxdUrl on iOS.
//
// It is not suitable for vusbUrl, which wraps payloads in multiplex frames —
// see DeviceBridge for that.
//
// Blocks until l fails or ctx is cancelled, then waits for the in-flight
// connections to finish.
func ServeRaw(ctx context.Context, l net.Listener, wsURL, sessionID, authB64 string) error {
	defer l.Close()
	...
	for {
		...
		connBridge := NewRawConnectionBridge(wsURL, sessionID, authB64, conn)
		wg.Add(1)
		go func() {
			defer wg.Done()
			connBridge.Run(ctx)
